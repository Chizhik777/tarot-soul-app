...
/**
 * TAROT SOUL APP v19.5 (Final Deploy Version)
 * - БАЗА: Supabase (URL: hvqdnasfjtbipuuvblbw)
 * - ДИЗАЙН: Белое мерцающее небо, моргающий кот.
 * - СТРУКТУРА: Оптимизировано для Vite + Vercel.
 */
...
// --- ЗВЕЗДНОЕ НЕБО (ТОЛЬКО БЕЛЫЕ ЗВЕЗДЫ) ---
...
export default function App() {
  ...
  const processBookings = (sorted) => {
    ...
    if (user.role === 'admin') {
      ...
      if (lastPendingCountRef.current !== null && pendingCount > lastPendingCountRef.current) { triggerMagicAlert(`Поток энергий: Новая заявка! ✨`); }
      lastPendingCountRef.current = pendingCount;
    }
    if (user.role === 'client') {
      const active = sorted.find(b => b.client_phone === user.phone && b.status !== 'completed');
      const prev = prevActiveBookingRef.current;
      if (prev && !active) { setSessionEndingOverlay(true); setClientHasNotification(false); playSound('notification'); }
      else if (active) {
        if (active.status === 'confirmed' && (!prev || prev.status === 'pending')) { triggerMagicAlert(`Запись подтверждена! ✨`); setClientHasNotification(true); }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`chat_${activeChatBooking.id}`).on('postgres_changes', { event: 'INSERT', table: 'messages', filter: `booking_id=eq.${activeChatBooking.id}` }, (payload) => {
      ...
      if (payload.new.sender !== (user.role === 'admin' ? 'master' : 'user')) playSound('notification');
      setTimeout(() => chatEndRef.current?.scrollIntoView({ behavior: 'smooth' }), 100);
    }).subscribe();
    return () => { supabase.removeChannel(channel); };
  ...
  const handleVerifyOtp = async () => {
    ...
    if (data) {
      localStorage.setItem('tarot_role', 'client'); localStorage.setItem('tarot_phone', safePhone);
      localStorage.setItem('tarot_name', data.name); localStorage.setItem('tarot_gender', data.gender);
      setUser({ role: 'client', phone: safePhone, ...data }); setView('home');
  ...
  const handleCompleteRegistration = async () => {
    ...
    if (!error) {
      localStorage.setItem('tarot_role', 'client'); localStorage.setItem('tarot_phone', safePhone);
      localStorage.setItem('tarot_name', clientName); localStorage.setItem('tarot_gender', clientGender);
      setUser({ role: 'client', ...profile }); setView('home');
    }
  };
  ...
  const submitBooking = async () => {
    ...
    triggerMagicAlert("Заявка отправлена! ✨"); setView('home');
  };

  const confirmBooking = async (id) => { handleInteraction(); await supabase.from('bookings').update({ status: 'confirmed' }).eq('id', id); };
  const endSession = async (id) => { handleInteraction(); await supabase.from('bookings').update({ status: 'completed' }).eq('id', id); setSessionEndingOverlay(true); playSound('notification'); };
  ...
  const handleCopyDonate = () => {
    handleInteraction(); const textArea = document.createElement("textarea"); textArea.value = "5599002127322628";
  ...
  return (
    <div className="fixed inset-0 bg-[#060608] flex items-center justify-center overflow-hidden font-sans text-white text-center">
      ...
      <div className="w-full h-[100dvh] max-w-md flex flex-col relative shadow-2xl overflow-hidden border-x border-white/5">
        <StarryBackground />
        {user && (
          <header className={`px-5 py-5 flex justify-between items-center backdrop-blur-2xl border-b flex-shrink-0 z-50 ${user.role === 'admin' ? 'bg-[#d4af37]/10 border-[#d4af37]/20' : 'bg-[#0d0d12]/95 border-white/[0.03]'}`}>
            <div className="flex items-center gap-4 text-left">
              {view !== 'home' && <button onClick={() => { if (view === 'chat' && user.role === 'admin') setView('admin-requests'); else if (view === 'archive-chat') setView(user.role === 'admin' ? 'admin-archive-list' : 'archive-list'); else setView('home'); }} className="p-1 -ml-1 text-[#d4af37] active:scale-125 transition-transform"><ChevronLeft size={24} /></button>}
              <div className="flex flex-col"><h1 className="text-[#d4af37] font-extralight tracking-[0.4em] uppercase text-[10px]">Tarot Soul</h1><span className="text-[6px] text-[#d4af37]/60 uppercase tracking-[0.2em] font-bold italic">Связь с Мастером</span></div>
            </div>
        ...
        <main className="flex-1 relative w-full overflow-hidden flex flex-col z-10">
          <AnimatePresence mode="wait">
            {!user ? (
              <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col items-center justify-center p-8 overflow-y-auto">
              ...
              <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 flex flex-col w-full overflow-hidden">
                ...
                {view === 'archive-list' && (<div className="flex-1 overflow-y-auto p-5 w-full bg-transparent"><h2 className="text-lg font-light uppercase tracking-widest mb-6 text-[#d4af37] font-mono italic text-center">История сеансов</h2><div className="space-y-4 pb-12">{allBookings.filter(b => b.client_phone === user.phone && b.status === 'completed').map(b => (<button key={b.id} onClick={() => { setSelectedArchiveBooking(b); setView('archive-chat'); }} className="w-full bg-[#16161f] border border-white/5 p-5 rounded-[30px] shadow-xl text-left active:scale-95 transition-transform flex flex-col gap-2"><div className="flex justify-between items-center w-full"><span className="text-[#d4af37] text-xs font-bold uppercase tracking-widest font-mono">{b.service}</span><span className="text-white/30 text-[9px] uppercase font-mono">{formatReadableDate(b.date)}</span></div><div className="text-white/50 text-[10px] font-light">Сеанс завершен Мастером</div></button>))}</div></div>)}
                {view === 'admin-archive-list' && (<div className="flex-1 overflow-y-auto p-5 w-full bg-transparent"><h2 className="text-lg font-light uppercase tracking-widest mb-6 text-[#d4af37] font-mono italic text-center">Архив Клиентов</h2><div className="space-y-4 pb-12">{allBookings.filter(b => b.status === 'completed').map(b => (<button key={b.id} onClick={() => { setSelectedArchiveBooking(b); setView('archive-chat'); }} className="w-full bg-[#16161f] border border-white/5 p-5 rounded-[30px] shadow-xl text-left active:scale-95 transition-transform flex flex-col gap-2"><div className="flex justify-between items-center w-full"><div className="flex items-center gap-2"><span className="text-[#d4af37] text-xs font-bold uppercase tracking-widest font-mono">{b.client_name}</span>{b.client_gender === 'female' ? <Venus size={10} className="text-[#ff4d4d]" /> : <Mars size={10} className="text-[#4d94ff]" />}</div><span className="text-white/30 text-[9px] uppercase font-mono">{formatReadableDate(b.date)}</span></div><div className="flex justify-between items-center w-full mt-1"><span className="text-white/60 text-[10px] font-light">{b.service}</span><span className="text-white/40 text-[9px] font-mono">{b.client_phone}</span></div></button>))}</div></div>)}
                {view === 'archive-chat' && selectedArchiveBooking && (<div className="flex-1 flex flex-col overflow-hidden text-white w-full bg-transparent relative text-center"><div className="px-4 py-3 bg-black/50 border-b border-white/[0.05] text-[9px] text-white/50 uppercase tracking-[0.2em] font-bold flex items-center justify-center gap-2 z-10 relative"><Archive size={14} className="text-white/30" /> {user.role === 'admin' ? `Архив: ${selectedArchiveBooking.client_name}` : `Архив: ${selectedArchiveBooking.service}`}</div><div className="flex-1 overflow-y-auto space-y-6 scrollbar-hide px-4 py-6 z-10 text-center">{archiveMessages.map(m => (<div key={m.id} className={`flex ${m.sender === 'master' ? 'justify-start' : 'justify-end'}`}><div className={`max-w-[85%] p-4 rounded-[22px] shadow-[0_10px_40px_rgba(0,0,0,0.7)] ${m.sender === 'master' ? 'bg-[#1a1a24]/95 text-white rounded-tl-none border border-[#d4af37]/30 text-left opacity-60' : 'bg-[#16161f]/95 text-white border border-white/10 rounded-tr-none shadow-lg text-left font-light opacity-60'}`}>{m.sticker_id ? (<div className="flex justify-center p-2 opacity-50">{(() => { const SIcon = STICKERS_MAP[m.sticker_id]; return SIcon ? <SIcon /> : null; })()}</div>) : m.image_url ? (<img src={m.image_url} alt="Insight" className="w-full h-auto rounded-xl mb-2 border border-black/10 opacity-50" />) : (<p className="text-[13px] font-light leading-relaxed italic whitespace-pre-wrap break-words">{m.text}</p>)}<span className={`text-[7px] opacity-40 mt-2 block text-right font-mono uppercase text-white/30`}>{m.time}</span></div></div>))}</div><div className="p-4 bg-black/90 border-t border-white/[0.05] flex-shrink-0 z-10"><p className="text-[9px] uppercase tracking-widest font-bold text-[#ff4d4d]/70 flex items-center justify-center gap-2"><Lock size={12} /> Сеанс закрыт для сообщений</p></div></div>)}
              </motion.div>
            )}
          </AnimatePresence>
        </main>

        <AnimatePresence>{sessionEndingOverlay && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[2000] bg-[#060608]/98 backdrop-blur-2xl flex flex-col items-center justify-center p-6 text-center"><button onClick={() => { setSessionEndingOverlay(false); setView(user?.role === 'admin' ? 'admin-requests' : 'home'); setActiveChatBooking(null); triggerMagicAlert("В Архиве ✨"); }} className="absolute top-6 right-6 text-white/30 p-3"><X size={28}/></button><motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }}><StickerZen /></motion.div><h2 className="text-[#d4af37] text-xl font-serif tracking-[0.3em] uppercase mt-8">Сеанс Завершен</h2><p className="text-white/50 text-xs font-light mt-4">Магический поток закрыт.<br/>Ответы сохранены в Архиве.</p><button onClick={() => { setSessionEndingOverlay(false); setShowDonateModal(true); setView('home'); }} className="mt-10 bg-[#ff4d4d]/10 text-[#ff4d4d] border border-[#ff4d4d]/30 px-6 py-4 rounded-[24px] flex items-center gap-3 font-bold uppercase tracking-widest text-[10px] active:scale-95 shadow-[0_0_20px_rgba(255,77,77,0.15)]"><Heart size={16} fill="#ff4d4d" /> Поблагодарить</button></motion.div>)}</AnimatePresence>

        <AnimatePresence>{showDonateModal && (<div className="fixed inset-0 z-[300] flex items-center justify-center p-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowDonateModal(false)} className="absolute inset-0 bg-black/95 backdrop-blur-md" /><motion.div initial={{ scale: 0.8, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-sm bg-[#1a1a24] border border-[#d4af37]/30 rounded-[3rem] p-8 shadow-2xl flex flex-col items-center"><button onClick={() => setShowDonateModal(false)} className="absolute top-6 right-6 text-white/30 p-2"><X size={20}/></button><div className="w-16 h-16 bg-[#ff4d4d]/10 rounded-full flex items-center justify-center mb-6"><motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ repeat: Infinity, duration: 1.5 }}><Heart size={32} className="text-[#ff4d4d]" fill="#ff4d4d" /></motion.div></div><h3 className="text-[#d4af37] text-base font-bold uppercase tracking-widest mb-2 font-serif">Энергообмен</h3><p className="text-white/60 text-[10px] font-light mb-8 max-w-[260px] mx-auto leading-relaxed">Моя цель — помогать. Если расклад был полезен, вы можете отправить благодарность по зову сердца.</p><div className="w-full bg-gradient-to-tr from-[#16161f] to-[#252535] border border-white/10 rounded-[24px] p-5 mb-6 shadow-xl"><p className="text-[8px] text-[#d4af37] uppercase tracking-widest font-bold font-mono mb-3 text-left">ЮMoney</p><p className="text-2xl font-mono text-white tracking-widest text-left font-light mb-1">5599 0021 2732 2628</p></div><button onClick={handleCopyDonate} className={`w-full font-bold py-5 rounded-[24px] uppercase text-[10px] tracking-widest active:scale-95 flex items-center justify-center gap-3 transition-all ${copied ? 'bg-green-500/20 text-green-400 border border-green-500/50' : 'bg-[#d4af37] text-black'}`}>{copied ? <><Check size={18}/> Скопировано</> : <><Copy size={18}/> Копировать номер</>}</button></motion.div></div>)}</AnimatePresence>

        <AnimatePresence>{showSupportModal && (<div className="fixed inset-0 z-[300] flex items-center justify-center p-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowSupportModal(false)} className="absolute inset-0 bg-black/95 backdrop-blur-md" /><motion.div initial={{ scale: 0.8, y: 20 }} animate={{ scale: 1, y: 0 }} className="relative w-full max-w-sm bg-[#1a1a24] border border-white/10 rounded-[3rem] p-8 shadow-2xl flex flex-col items-center"><button onClick={() => setShowSupportModal(false)} className="absolute top-6 right-6 text-white/30 p-2"><X size={20}/></button><div className="w-16 h-16 bg-white/5 rounded-full flex items-center justify-center mb-6"><Bug size={24} className="text-white/40" /></div><h3 className="text-white/80 text-base font-bold uppercase tracking-widest mb-2 font-serif">Поддержка</h3><p className="text-white/40 text-[10px] font-light mb-8 mx-auto leading-relaxed">Заметили техническую ошибку? Напишите разработчику.</p><button onClick={handleSupportEmail} className="w-full bg-[#16161f] text-white/80 border border-white/10 font-bold py-4 rounded-[24px] uppercase text-[9px] tracking-widest active:scale-95 shadow-xl flex items-center justify-center gap-3 hover:border-[#d4af37]/50"><Mail size={16}/> Написать на Email</button></motion.div></div>)}</AnimatePresence>
        
        <AnimatePresence>{showClientInfo && activeChatBooking && (<div className="fixed inset-0 z-[150] flex items-center justify-center p-6"><motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setShowClientInfo(false)} className="absolute inset-0 bg-black/98 backdrop-blur-md" /><motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} className="relative w-full max-w-sm bg-[#16161f] border-2 border-[#d4af37]/40 rounded-[3rem] p-10 shadow-2xl"><div className="w-16 h-16 bg-[#d4af37]/10 rounded-full flex items-center justify-center mx-auto mb-6"><CalendarCheck size={32} className="text-[#d4af37]" /></div><h3 className="text-[#d4af37] text-base font-bold uppercase tracking-widest mb-4">Ваша запись</h3><div className="space-y-3 mb-8 text-white/80 font-light"><div className="bg-white/5 p-4 rounded-2xl"><p className="text-[8px] text-[#d4af37]/50 uppercase tracking-widest mb-1 font-mono">Дата</p><p className="text-lg font-mono">{formatReadableDate(activeChatBooking.date)}</p></div><div className="bg-white/5 p-4 rounded-2xl"><p className="text-[8px] text-[#d4af37]/50 uppercase tracking-widest mb-1 font-mono">Время</p><p className="text-xl font-bold text-[#d4af37] font-mono">{activeChatBooking.time}</p></div></div><button onClick={() => { setShowClientInfo(false); setView('chat'); }} className="w-full bg-[#d4af37] text-black font-bold py-4 rounded-2xl uppercase text-[10px] active:scale-95 shadow-xl">Открыть чат</button></motion.div></div>)}</AnimatePresence>
      </div>
    </div>
  );
}